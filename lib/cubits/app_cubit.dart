...
class AppState {
  ...
  AppState copyWith({
    ...
    return AppState(
      ...
      totalGroupContribution: totalGroupContribution ?? this.totalGroupContribution,
...
class AppCubit extends Cubit<AppState> {
  ...
  void _fetchDataFromFirebase() {
    ...
    final sub1 = _firestore.doc('admin/account_number').snapshots().listen((doc) {
      if (doc.exists) {
    ...
    final sub2 = _firestore.doc('stats/contributions').snapshots().listen((doc) {
      if (doc.exists) {
        final total = ((doc.data()?['totalAmount'] ?? 0) as num).toDouble();
    ...
    final sub3 = _firestore.collection('users').doc(currentUserId).snapshots().listen((doc) {
      if (doc.exists) {
        final myContrib = ((doc.data()?['myContribution'] ?? 0) as num).toDouble();
    ...
    final sub4 = _firestore.collection('users').snapshots().listen((query) {
      final members = query.docs.map((doc) => {'id': doc.id, ...doc.data()}).toList();
    ...
    final sub5 = _firestore.collection('dues')
  ...
  Future<void> uploadReceiptAndSubmitDue(XFile file, double amount) async {
    ...
    try {
      ...
      final ref = _storage.ref().child('receipts/${DateTime.now().millisecondsSinceEpoch}_${file.name}');
      ...
      emit(state.copyWith(isLoading: false, successMessage: 'Receipt uploaded successfully! Waiting for admin confirmation.'));
    } catch (e) {
      emit(state.copyWith(isLoading: false, errorMessage: 'Failed to upload receipt: $e'));
    }
  }
